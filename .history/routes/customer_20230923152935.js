...
customerRouter.post("/profile/:id", async (req, res) => {
    ...
    // Exclude current user's id when checking for existing email
    const existedEmail = await Customer.findOne({ email, _id: { $ne: req.params.id } });
    if (existedEmail) {
        errors.push("This email was taken !!");
    }

    // Exclude current user's id when checking for existing phone
    const existedPhone = await Customer.findOne({ phone, _id: { $ne: req.params.id } });
    if (existedPhone) {
